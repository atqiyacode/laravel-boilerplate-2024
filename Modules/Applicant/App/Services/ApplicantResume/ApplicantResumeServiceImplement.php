...
namespace Modules\Applicant\App\Services\ApplicantResume;

use Modules\Applicant\App\Http\Resources\ApplicantResume\ApplicantResumeResource;
use Modules\Applicant\App\Http\Resources\ApplicantResume\PublicApplicantResumeResource;
use Modules\Applicant\App\Http\Resources\ApplicantResume\SimpleApplicantResumeResource;
use LaravelEasyRepository\ServiceApi;
use Illuminate\Support\Facades\DB;
use Modules\Applicant\App\Repositories\ApplicantResume\ApplicantResumeRepository;

class ApplicantResumeServiceImplement extends ServiceApi implements ApplicantResumeService
{
    ...
    public function __construct(ApplicantResumeRepository $mainRepository)
    {
    ...
    public function getPaginate()
    {
        ...
        return $this->setMessage(__('alert.success-found'))
            ->setStatus(true)
            ->setCode(200)
            ->setResult(SimpleApplicantResumeResource::collection($response));
    }
    ...
    public function findById($id)
    {
        ...
        return $this->setMessage(__('alert.success-found'))
            ->setStatus(true)
            ->setCode(200)
            ->setResult(new ApplicantResumeResource($response));
    }

    public function findByUserId($userId)
    {
        $response = $this->mainRepository->findByUserId($userId);
        return $this->setMessage(__('alert.success-found'))
            ->setStatus(true)
            ->setCode(200)
            ->setResult(new PublicApplicantResumeResource($response));
    }
    ...
    public function create($data)
    {
        ...
        return $this->setMessage($message)
            ->setStatus(true)
            ->setCode(201)
            ->setResult(new SimpleApplicantResumeResource($response));
    }
