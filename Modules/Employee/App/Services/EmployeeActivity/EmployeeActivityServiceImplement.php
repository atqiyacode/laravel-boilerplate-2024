...
class EmployeeActivityServiceImplement extends ServiceApi implements EmployeeActivityService
{
    ...
    public function __construct(EmployeeActivityRepository $mainRepository)
    {
    ...
    public function getPaginate()
    {
        ...
        return $this->setMessage(__('alert.success-found'))
            ->setStatus(true)
            ->setCode(200)
            ->setResult(EmployeeActivityResource::collection($response));
    }

    public function getPaginateByEmployeeId($employeeId)
    {
        $response = $this->mainRepository->getPaginateByEmployeeId($employeeId);
        return $this->setMessage(__('alert.success-found'))
            ->setStatus(true)
            ->setCode(200)
            ->setResult(EmployeeActivityResource::collection($response));
    ...
    public function findById($id)
    {
        ...
        return $this->setMessage(__('alert.success-found'))
            ->setStatus(true)
            ->setCode(200)
            ->setResult(new EmployeeActivityResource($response));
    }
