...
class EmployeeServiceImplement extends ServiceApi implements EmployeeService
{
    ...
    public function getPaginate()
    {
        ...
        return $this->setMessage(__('alert.success-found'))
            ->setStatus(true)
            ->setCode(200)
            ->setResult(EmployeeResource::collection($response));
    }
    ...
    public function getContactPaginate()
    {
        ...
        return $this->setMessage(__('alert.success-found'))
            ->setStatus(true)
            ->setCode(200)
            ->setResult(ContactsResource::collection($response));
    }
    ...
    public function findById($id)
    {
        ...
        return $this->setMessage(__('alert.success-found'))
            ->setStatus(true)
            ->setCode(200)
            ->setResult(new EmployeeResource($response));
    }

    public function findByNik($nik)
    {
        $response = $this->mainRepository->findByNik($nik);
        return $this->setMessage(__('alert.success-found'))
            ->setStatus(true)
            ->setCode(200)
            ->setResult(new EmployeeResource($response));
