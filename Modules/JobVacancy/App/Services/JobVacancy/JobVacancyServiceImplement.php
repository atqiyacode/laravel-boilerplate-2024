...
namespace Modules\JobVacancy\App\Services\JobVacancy;

use Modules\JobVacancy\App\Http\Resources\JobVacancy\JobVacancyResource;
use Modules\JobVacancy\App\Http\Resources\JobVacancy\PublicJobVacancyResource;
use LaravelEasyRepository\ServiceApi;
use Illuminate\Support\Facades\DB;
use Modules\JobVacancy\App\Repositories\JobVacancy\JobVacancyRepository;

class JobVacancyServiceImplement extends ServiceApi implements JobVacancyService
{
    ...
    public function __construct(JobVacancyRepository $mainRepository)
    {
    ...
    public function getPaginate()
    {
        ...
        return $this->setMessage(__('alert.success-found'))
            ->setStatus(true)
            ->setCode(200)
            ->setResult(JobVacancyResource::collection($response));
    }

    public function getPaginateCareer()
    {
        $response = $this->mainRepository->getPaginateCareer();
        return $this->setMessage(__('alert.success-found'))
            ->setStatus(true)
            ->setCode(200)
            ->setResult(PublicJobVacancyResource::collection($response));
    }
    ...
    public function findById($id)
    {
        ...
        return $this->setMessage(__('alert.success-found'))
            ->setStatus(true)
            ->setCode(200)
            ->setResult(new JobVacancyResource($response));
    }

    public function findBySlug($slug)
    {
        $response = $this->mainRepository->findBySlug($slug);
        return $this->setMessage(__('alert.success-found'))
            ->setStatus(true)
            ->setCode(200)
            ->setResult(new PublicJobVacancyResource($response));
    }
